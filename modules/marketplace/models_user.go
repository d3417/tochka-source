...
func (u User) Store() *Store {
	store, _ := CacheFindStoreByUserUuid(u.Uuid)
	return store
}
...
func cacheFindUserByUuid(uuid string) (*User, error) {
	var (
		user *User
		err  error
	)

	key := "user-by-uuid-" + uuid

	if cUser, _ := cache1m.Get(key); cUser == nil {
		user, _ = findUserByUuid(uuid)
		cache1m.Set(key, user)
	} else {
		user = cUser.(*User)
...
func cacheFindUserByUsername(username string) (*User, error) {
	var (
		user *User
		err  error
	)

	key := "user-by-username-" + username
